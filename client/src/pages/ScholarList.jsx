...
export const Scholarlist = () => {
    ...
    return (
        <div>
            ...
            <h1 className="text-4xl text-center m-5 font-bold">Scholarship Opportunities</h1>
            <p className="text-center">Scholarship Related information opportunities for Specially Abled Students</p>
                <button className="text-center m-auto text-violet-800 font-bold text-xl p-2 ">Apply Now</button>
                </div>
                <img className="md:w-2/5 sm:2/4 w-3/5 m-auto  sm:m-11  rounded h-96" src="https://images.pexels.com/photos/267885/pexels-photo-267885.jpeg?auto=compress&cs=tinysrgb&w=600" alt="job" />
               ...
               <div className="flex flex-wrap mb-10">
                    {scholar.map((scholar, index) => (
                         <div className="w-1/2 sm:w-1/3 md:w-1/4 p-4" key={index}>
                          ...
                          <img className="w-full h-44 rounded object-cover object-center" src={scholar.Logo} alt="logo" />
                            <div className="p-4">
                          {scholar.Type==='private' && <h2 className="font-bold font-serif text-xl mb-1">{scholar.company}</h2>}
                          <p className="mb-1 font-semibold">{scholar.Title}</p>
                          <p className="mb-1 font-semibold">{scholar.Type}</p>
                          {scholar.Type==='private' && <p className="font-bold font-serif text-xl mb-1">{scholar.location}</p>}
                          <p className="mb-2 flex justify-between"><p>Apply Deadline:</p>{new Date(scholar.applyDeadline).toLocaleDateString()}</p>
